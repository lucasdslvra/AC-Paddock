...
export default async function SoireePage() {
  ...
  const [soiree, voterCount, memberCount, actor] = await Promise.all([
    ...
    // Le dénominateur de « 5 / 8 ont voté » : les membres connus de la base, c'est-à-dire
    // ceux qui ont déjà écrit quelque chose. Le serveur Discord en compte sans doute
    // plus, mais l'application ne les a jamais vus.
    prisma.user.count(),
